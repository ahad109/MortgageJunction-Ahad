...
export default class Lookup extends NavigationMixin(LightningElement) {
    ...
    setSearchResults(results) {
     try {
         console.log('Search Results in SetSearchResults = ' + results)
            // Reset the spinner
        this.loading = false;
        ...
        this.searchResultsLocalState = this._searchResults.map((result, i) => {
         ...
         console.log('Error in setSearchResults' + JSON.stringify(error));
     }
    }
    ...
    hasSelection() {
        return (this._curSelection.length > 0 && this._curSelection.title !='');
    }
    ...
    get getInputClass() {
        ...
        if ( (this._isDirty && this.required && this._curSelection.title!='' &&!this.hasSelection())) {
            css += 'has-custom-error ';
        }
        if (!this.isMultiEntry) {
